...
import { GetMyTodosQuery, RemoveTodoMutation, RemoveTodoMutationVariables ,Todos, ToggleTodoMutation, ToggleTodoMutationVariables  } from '../../generated/graphql';
...
const TodoItem = ({index, todo}: TodoItemType) => {
  ...
  const [todoRemove] = useMutation<RemoveTodoMutation, RemoveTodoMutationVariables>(REMOVE_TODO, {
    update(cache, { data }) {
      const existingTodos = cache.readQuery<GetMyTodosQuery>({ query: GET_MY_TODOS });
